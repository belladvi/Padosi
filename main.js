/* Padosi — scroll film engine (from scroll-site-generator, rebranded)
...
function transformBase(el) {
  if (el.classList.contains("cap-hero")) return "translate(0, 0)";
...
function devPlaceholder(msg) {
  ...
  const draw = () => {
    ...
    g.addColorStop(0, "#161009");
    g.addColorStop(1, "#0e0906");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = "rgba(192,96,62,0.9)";
...
loadManifest()
  ...
  .catch(() => devPlaceholder("frames not built yet"));
